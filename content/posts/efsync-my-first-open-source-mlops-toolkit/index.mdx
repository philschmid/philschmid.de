...
**\*Note**: If you sync a file with SCP from a local directory (e.g. `model/bert`) to efs (`my_efs_model`) efsync will
sync the model to `my_efs_model/bert` that happens because scp uploads the files recursively.\*
...
- `efs_filesystem_id`: the AWS EFS filesystem id (moint point).
